...
def scan_load_basic_data():
    ...
    # load rnaseq data
    try:
        rnaseq_coverage_data = pickle.load(open(root_path + '/rnaseq_coverage_data.p', 'rb'))
        ...
        pickle.dump(rnaseq_coverage_data, open(root_path + '/rnaseq_coverage_data.p', 'wb'))
...
def scan2fig(_chr, scan_start, scan_end, pos, strand, prediction_scan, scan_cage_cov, scan_rnaseq_cov, reducenoise=False):
    def minmax(data):
        tmp = []
        if max(data)-min(data)==0:
            for x in data:
    ...
    def reduce_noise(data):
        tmp = [np.mean([data[0], data[1], data[2]]), np.mean([data[0], data[1], data[2], data[3]])]
        for i in range(2, len(data) - 2):
            tmp.append(np.mean([data[i - 2], data[i - 1], data[i], data[i + 1], data[i + 2]]))
        tmp.append(np.mean([data[-4], data[-3], data[-2], data[-1]]))
        tmp.append(np.mean([data[-3], data[-2], data[-1]]))
        return tmp

    plt.figure(figsize=[30, 9], dpi=100)
    sns.set()
    tss_pred = [x[0] for x in prediction_scan]
    if reducenoise==True:
        sns.lineplot(range(len(tss_pred)), reduce_noise(tss_pred), color='grey', label='TSS Prediction')
    else:
        sns.lineplot(range(len(tss_pred)), tss_pred, color='grey', label='TSS Prediction')
    sns.lineplot(range(len(scan_cage_cov)), minmax(scan_cage_cov), color='red', label='CAGE Coverage')
    sns.lineplot(range(len(scan_rnaseq_cov)), minmax(scan_rnaseq_cov), color='orange', label='RNA-Seq Coverage')
    plt.legend()
    plt.title('{}:{}...{};{};{}'.format(_chr, scan_start, scan_end, strand, pos))
    #plt.vlines(int(pos-scan_start), 0, 1, colors="r", linestyles="dashed")
    plt.xlim(0, int(scan_end-scan_start))
...
def scan(sess, model, _chr='chr1', scan_start=1, scan_end=10001, pos=100, strand='+',mm=False,reducenoise=False):
    ...
    # save scan figure
    #scan2fig(_chr, scan_start, scan_end, pos, strand, prediction_scan, scan_cage_cov, scan_rnaseq_cov,reducenoise)

    pickle.dump(prediction_scan, open(scanfig_path + '/{}:{}_{}.p'.format(_chr,scan_start,scan_end), 'wb'))
...
if __name__ == '__main__':
        ...
        file_id = sys.argv[1]
        root_path = sys.argv[2] #'colon'
        load_model_path = sys.argv[3] #rep_path + '/basic_model/model_best_f1.npz'
        scanfig_path = root_path + sys.argv[4] #'/scanall_genebed_cov'  # scanall_genebed ~ all protein coding genes
        os.system('mkdir -p {}'.format(scanfig_path))
        scanbed = '{}.{}.bed'.format(
            sys.argv[5],file_id) #'scanall_genebed_cov/colon.carcinoma.RNAseq.bedgraph.merged.{}.bed'.format(file_id)
        ...
        with open(scanbed,'r') as f:
            ...
            for line in f:
                ...
                strand = line[5]
                if os.path.isfile(scanfig_path + '/{}:{}_{}.p'.format(_chr,scan_start,scan_end)):
                    _info = '[info]\tscan /{}:{}_{}.p exists'.format(_chr,scan_start,scan_end)
                    print(_info)
                    pass
                else:
                    #try:
